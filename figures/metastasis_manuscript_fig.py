...
if __name__ == '__main__':
    ...
    plt.xlabel("Weeks", fontsize=6)
    plt.ylabel("Tumour Volume $mm^3$", fontsize=6)
    plt.xticks(fontsize=6)
    plt.yticks(fontsize=6)
    plt.xlim(0, 30)
    plt.ylim(0, max_tumour_volume)
    box = ax.get_position()
    ax.set_position([box.x0 + box.height * 0.015, box.y0 + box.height * 0.05, box.width * 0.85, box.height * 0.95])
    plt.title("Tumours with metastasis", weight='bold', fontsize=8)
    ...
    plt.axvline(x=14, color='gray', linestyle='--', linewidth=1)
    sns.despine()
    plt.legend(bbox_to_anchor=(1.325, 1), frameon=False, fontsize=6)
    plt.xlabel("Weeks", fontsize=6)
    plt.ylabel("Tumour Volume $mm^3$", fontsize=6)
    plt.xticks(fontsize=6)
    plt.yticks(fontsize=6)
    ...
    box = ax.get_position()
    ax.set_position([box.x0 + box.height * 0.015, box.y0 + box.height * 0.05, box.width * 0.85, box.height * 0.95])
    plt.title("Tumours without metastasis", weight='bold', fontsize=8)
    ...
    sns.heatmap(fig1_markers, cmap=cmap, cbar=False)
    ...
    box = ax.get_position()
    ax.set_position([box.x0 + box.width * 0.15, box.y0 + box.height * 0.15, box.width * 0.85, box.height * 0.85])
    ...
    cmap = ["#5a0064d9", "#fb8811d9"]
    sns.heatmap(fig8_markers, cmap=cmap, cbar=False)
    legend_handles = [Patch(color=cmap[True], label='Positive'),
                      Patch(color=cmap[False], label='Negative')]
    plt.legend(handles=legend_handles, ncol=2, bbox_to_anchor=[0.5, 1.0], loc='lower center',
               frameon=False, fontsize=6)
    ...
    sns.swarmplot(x="group", y="H-score", data=fig8_919, linewidth=1, ax=ax, size=5)
    sns.despine()
    plt.ylim(0, 200)
    plt.xticks(fontsize=6)
    plt.yticks(fontsize=6)
    ...
    plt.xticks(rotation=90, fontsize=6)
    plt.yticks(fontsize=6)
    plt.xlabel("PDX", fontsize=6)
    plt.ylabel("Ki67 Percentage", fontsize=6)
    box = ax.get_position()
