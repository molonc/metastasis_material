...
get_statistical_test_gene_types_per_chr <- function(){
  git_dir <- '/Users/hoatran/Documents/projects_BCCRC/hakwoo_project/code/metastasis_material/supp_tables/'
  save_dir <- '/Users/hoatran/Documents/projects_BCCRC/hakwoo_project/code/results_bulkRNAseq/SA919_full/'
  datatag <- 'SA919_full'
  ...
  exp_df <- data.table::fread(paste0(save_dir, subtag, '/', paste(obs_conds, collapse='_vs_'), '_total_exp.csv.gz'))
  dim(exp_df)
  resAC <- get_stat_per_chr(exp_df, obs_conds)
  resAC
  ...
  exp_df <- data.table::fread(paste0(save_dir, subtag, '/', paste(obs_conds, collapse='_vs_'), '_total_exp.csv.gz'))
  dim(exp_df)
  resBC <- get_stat_per_chr(exp_df, obs_conds)
  resBC
  res_total <- dplyr::bind_rows(resAB, resAC, resBC)
  data.table::fwrite(res_total, paste0(git_dir, 'SuppTable71_stat_panelB.csv'))
