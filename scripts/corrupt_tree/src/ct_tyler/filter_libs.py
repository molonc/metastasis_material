...
def filter_libs(**kwargs):
    ...
    if jira_id:
        ...
        if len(analyses) == 1:
            ...
            print("Sample type is: %s" % sample_type)
            # cell_call_filter = "C2"
...
#             filtered_reads = reads_df.loc[(reads_df['map'] >= 0.99) & (reads_df['valid'])]
            ...
            # print('______Before remove low mapped reads: ')
            # print(cell_predict_df.shape)
            cell_predict_df = cell_predict_df[cell_predict_df['total_mapped_reads'] >= 10000]
            # print('______After remove low mapped reads: ')
            # print(cell_predict_df.shape)
            # Hoa: add more filter condition to remove mouse cells
            # print('______Before remove mouse cells: ')
            # print(cell_predict_df.shape)
            cell_predict_df = cell_predict_df[cell_predict_df['fastqscreen_mm10'] < cell_predict_df['fastqscreen_grch37']]
            # print('______After remove mouse cells: ')
            # print(cell_predict_df.shape)
            ...
            if not (filtered_reads.empty and filtered_metrics.empty and not_s_phase.empty):
                ...
                if not filtered_state_data.empty:
                    ...
                    print('______Final output for library id: %s' % library_id)
