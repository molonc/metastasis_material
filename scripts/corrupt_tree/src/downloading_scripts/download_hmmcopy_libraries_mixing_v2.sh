...
input_lib_ticket=/home/htran/Projects/hakwoo_project/metastasis_material/scripts/corrupt_tree/src/downloading_scripts/jira_tickets_mixing_exp.txt

IFS=$'\n' read -d '' -r -a lib_jira_tickets < $input_lib_ticket




for ((n=0;n<${#lib_jira_tickets[@]};n++));
do
  echo "________________________________________________________"
  printf "Testing jira 1: %s\n" "${lib_jira_tickets[n]}"
  #echo "${lib_jira_tickets[n]}" | cut -d "," -f 1
  #echo "${lib_jira_tickets[n]}" | cut -d "," -f 2
  library_id=$(echo "${lib_jira_tickets[n]}" | cut -d "," -f 1 )
  jira_ticket=$(echo "${lib_jira_tickets[n]}" | cut -d "," -f 2 )
  echo "$library_id"
  echo "$jira_ticket"
  ...
  out_fn1="${output_hmmcopy_fd}${library_id}_reads.csv.gz"
  ...
  out_fn11="${output_hmmcopy_anno}${library_id}_metrics.csv.gz"
