...
make_left_annot <- function(copynumber, clones) {
    ...
    annot_colours$Library <- make_discrete_palette("Set2", library_levels)
    ...
    if(!is.null(clones)) {
        ...
        left_annot <- HeatmapAnnotation(
            ...
            which="row", annotation_width=unit(rep(0.4, 3), "cm"),
            annotation_legend_param=list(
                Clone=list(nrow=clone_legend_rows),
                Library=list(nrow=library_legend_rows)
            )
        )
    } else {
        left_annot <- HeatmapAnnotation(
            Library=library_labels, col=annot_colours,
...
make_copynumber_heatmap <- function(copynumber, clones) {
...
make_cell_copynumber_tree_heatmap <- function(tree, copynumber, clones) {
    ...
    copynumber_hm <- make_copynumber_heatmap(copynumber, clones)
...
main <- function() {
    ...
    make_cell_copynumber_tree_heatmap(tree, copynumber, clones)
    dev.off()
}
