...
plot_CNV_corrupt_tree <- function(df_cnv, clones){
  ...
  df_cnv <- df_cnv %>% 
    dplyr::select(all_of(c(clones, 'chr_desc'))) %>% 
...
## at clonal level
get_median_LOH_genotype_v2 <- function(ascn_fn, 
  ...
  # cells_use <- intersect(cell_clones$cell_id, unique(ascn$cell_id))
  # cell_clones <- cell_clones %>% 
  ...
  median_ascn <- ascn %>%
    dplyr::group_by(clone_id, chr, start, end) %>% #chr_desc
    dplyr::summarise(cnv=median(state),
                     LOH = calc_mode(LOH)) %>%
    ungroup()  %>%
    dplyr::mutate(chr_desc=paste0(chr,'_',start,'_',end))
  dim(median_ascn)
  length(unique(median_ascn$chr_desc))
  # ascn <- get_chr_infos(ascn)
  print(dim(median_ascn))
  # ascn1 <- ascn1 %>%
  #   dplyr::mutate(LOH_event = ifelse(cnv==2,'cnLOH',
  #                                    ifelse(cnv<2,'LOH', 'ampLOH')))
  
  median_ascn <- median_ascn %>%
    dplyr::mutate(LOH_event = 
                    case_when(
                      LOH=='LOH' & cnv==2 ~ 'cnLOH',
                      LOH=='LOH' & cnv<2 ~ 'LOH',
  ...
  data.table::fwrite(median_ascn, paste0(save_dir,datatag, '_median_genotype_ascn.csv.gz'))
  return(res)
}
# LOH (loss of heterozygosity) allele copy number analysis 
# involves identifying and quantifying the number of copies 
# of each parental allele at a given genomic locus, 
# rather than just the total number of copies. 
# This technique, also known as allele-specific copy number analysis (ASCN), 
# is crucial for detecting copy number changes in cancer and other genetic disorders, 
# especially events like copy-neutral LOH (CN-LOH), which are missed by total copy number analysis 
# but are significant for understanding cancer progression and identifying potential therapeutic targets.

detect_LOH_change_v2 <- function(copynumber_fn, median_ascn, save_dir, datatag){
  ...
  # rm(copynumber)
  length(filtered_bins) #4375 bins
  ...
  dim(median_ascn)
  ## Get regions with change, and check for changes in ascn as well
...
obs_clones <- c('A','B')
viz_LOH_events_v2 <- function(filtered_bins, df_cnv, obs_clones, save_dir){
  if(!dir.exists(save_dir)){
    dir.create(save_dir)
  }
  filtered_bins <- intersect(unique(df_cnv$chr_desc), filtered_bins)
  df_cnv <- df_cnv %>%
    dplyr::filter(clone_id %in% obs_clones
                  & chr_desc %in% filtered_bins)
  # CNV plot corrupt tree input data 
  df_cnv2 <- df_cnv %>%
    # dplyr::filter(LOH_event !='others') %>%
    dplyr::select(chr_desc,clone_id,cnv) %>%
    dplyr::filter(chr_desc %in% filtered_bins) %>%
    tidyr::pivot_wider(names_from = 'clone_id', values_from='cnv') %>%
    tidyr::drop_na(obs_clones[1],obs_clones[2]) %>%
    as.data.frame()
  
  dim(df_cnv2)
  median_cnv_plt <- plot_CNV_corrupt_tree(df_cnv2, obs_clones)
...
get_color_legend_ascn <- function(plotcol){
  source('/Users/hoatran/Documents/projects_BCCRC/hakwoo_project/code/results_CNA/ascn_SA919/scripts/col_palettes.R')
