...
sample_run_fast <- function(ctdir = NULL, just_normal = FALSE, use_greedy_tree = FALSE, ...) {
  if (!is.null(ctdir)) {
    ...
    corrupt_input <- file.path(ctdir, 'filtered.csv')
...
cn2binary_newencoding <- function(cnv_path, output_file, prob=.90, probAVG=.90) {
  # TODO: compute average ploidy too for each cell
...
cn2binary_newencoding(opt$input, opt$outfile, as.numeric(opt$filterthrs), .90)
