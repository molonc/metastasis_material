...
#                 total_state_samples = total_state_samples.join(filtered_state_data.loc[mask.values, :])
                total_state_samples = total_state_samples.join(filtered_state_data.loc[mask.values, :],how='inner')
...
def get_noisy_cells(cnv_df, output_file, pb=0.8):
...
#     print("Statistic mtx: {0} {1}".format(cnv_stat.shape[0],cnv_stat.shape[1]))
    cells_rv_df = cnv_stat.loc[np.invert(cnv_stat['is_noise']),:]
    if not cells_rv_df.empty:
