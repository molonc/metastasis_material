...
get_library_grouping <- function(cell_ids, grouping_file) {
    ...
    groupings <- read_tsv(grouping_file)
    if('sample_id' %in% colnames(groupings)){
        groupings$sample <- groupings$sample_id
        groupings$library_id <- groupings$grouping
    }
    # colnames(groupings)[which(names(groupings) == "grouping")] <- "library_id"
    sample_ids <- get_sample_id(cell_ids)
    tmp <- data.frame(cell_id=cell_ids, library_id=library_ids, sample=sample_ids, stringsAsFactors=FALSE)
    tmp <- left_join(tmp, groupings, by = c("library_id","sample"))  #composite key
    # print("DEBUG")
...
# copynumber_fn <- '/home/htran/storage/gm_instability_results/HEK293/total_merged_filtered_states.csv'
# tree_fn <- '/home/htran/storage/gm_instability_results/HEK293/tree.newick'
# clones_fn <- '/home/htran/storage/gm_instability_results/HEK293/graph_cut/cell_clones.csv'
# output_fn <- '/home/htran/storage/gm_instability_results/HEK293/graph_cut/cell_cn_tree_heatmap.png'
# grouping_file <- '/home/htran/storage/gm_instability_results/HEK293/library_groupings.csv'
# brlen <- NULL


# tree_fn <- '/home/htran/storage/datasets/drug_resistance/dlp_results/SA535/SA535_total_v2/tree.newick'
# cn_fn <- '/home/htran/storage/datasets/drug_resistance/dlp_results/SA535/SA535_total_v2/total_merged_filtered_states.csv'
# tree <- read.tree(tree_fn)
# grouping_file <- '/home/htran/storage/datasets/drug_resistance/dlp_results/SA535/SA535_total_v2/library_groupings.csv'
# copynumber <- read.csv(cn_fn, header=T, row.names=1, check.names = F,stringsAsFactors = FALSE)
# clones <- read_tsv('/home/htran/storage/datasets/drug_resistance/dlp_results/SA535/SA535_total_v2/cell_clones.csv')
# Rscript /home/htran/Projects/farhia_project/rscript/dlp/corrupt_tree/src/tree_viz/make_cell_copynumber_tree_heatmap.R 
# -t  
# -cn  

#     -grouping_file 
# -o /home/htran/storage/datasets/drug_resistance/dlp_results/SA535/SA535_total_v2/cell_cn_tree_heatmap_updated_color.png
# library_grouping.csv should contain a column grouping or library_id, and sample, treatment_st, passage, PDX
# @Sohrab: example of snakemake script:
