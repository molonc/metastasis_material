...
def filter_libs(**kwargs):
    ...
    if jira_id:
        ...
        if len(analyses) == 1:
            ...
            print("******   Hoa, DEBUG 1: sample type and cell call filter: ")
            print("******   sample type %s " % sample_type)
            print("******   cell call filter %s " % cell_call_filter)
            # print("******   filter_cond %s " % filter_cond)
            ...
            print("******   Hoa, DEBUG 2: sample type and cell call filter: ")
            print('debug metrics: %s' % kwargs['metrics'])
            unique, counts = np.unique(filtered_metrics['cell_call'], return_counts=True)
            print(dict(zip(unique, counts)))
            print("cell_call values here are different from debug 1")
            
            filtered_metrics = filtered_metrics[(filtered_metrics['cell_call'] == cell_call_filter)]
            
            print("so the filtered output will be empty because cell call values are different from cell_call_filter")
            print('Is filtered_metrics is empty? ')
            print(filtered_metrics.empty)
...
if __name__ == '__main__':
    ...
    # Need to download data first, then run args above
    library_id = 'A98293A'
    samples = 'SA919X7XB05608'
