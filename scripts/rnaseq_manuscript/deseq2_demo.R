...
## Input data: dim(cts) head(cts): samples are in the columns, and genes are in row names. class(cts): a mtx
# head(cts)
...
## coldata, class(coldata): a data frame, with row names are samples name, and columns are features
# coldata, condition is one of the column
#           condition        type
...
counts_df <- counts(dds)
norm_df <- counts(dds, normalized=TRUE)
dim(counts_df)
dim(norm_df)
colnames(counts_df)
sc <- colSums(counts_df)
sn <-  colSums(norm_df)

stat <- tibble::tibble(library_size=c(sc, sn), sample_type=c(names(sc), names(sn)),
                       processed=c(rep('counts',length(sc)), rep('normalized',length(sn))))
...
library("dplyr")
input_dir <- '~/Documents/BCCRC_projects/hakwoo_project/code/metastasis_material/materials/bulkRNAseq/'
raw_df <- data.table::fread(paste0(input_dir, 'SA919/SA919_total_raw_counts.csv.gz'))
metasample_df <- data.table::fread(paste0(input_dir, 'SA919/library_groupings_bulk_SA919_cloneIds.csv'))
dim(raw_df)  
# View(metasample_df)
# sum(metasample_df$sample_id %in% colnames(raw_df))
raw_df <- raw_df %>%
...
## C met vs B pri
metasampleBpri <- metasample_df %>%
  dplyr::filter(clone_id=='B' & mainsite=='Primary')
metasampleBmet <- metasample_df %>%
  dplyr::filter(clone_id=='B' & mainsite=='Metastasis')
metasampleCmet <- metasample_df %>%
  dplyr::filter(clone_id=='C' & mainsite=='Metastasis')

metasampleCmet1 <- metasampleCmet
rownames(metasampleCmet1) <- paste0(rownames(metasampleCmet1),'_2')
colnames(metasample_CB)
metasample_CB <- dplyr::bind_rows(metasampleCmet, metasampleCmet1)
metasample_CB <- dplyr::bind_rows(metasample_CB, metasampleBpri)
metasample_CB <- metasample_CB %>%
  dplyr::select(mainsite, clone_id) #, clone_id
t <- raw_df[,rownames(metasampleCmet), drop=F]
dim(t)
colnames(t) <- rownames(metasampleCmet1)
dim(raw_df)
length(unique(rownames(raw_df)))
length(unique(rownames(t)))
length(unique(rownames(raw_CB)))
colnames(raw_CB)
raw_CB <- dplyr::bind_cols(raw_df, t)
raw_CB <- raw_CB[,rownames(metasample_CB)]
raw_CB <- round(raw_CB, 0)
head(raw_CB)
dim(raw_CB)
dim(metasample_CB)
dds_CB <- DESeqDataSetFromMatrix(countData = raw_CB,
                              colData = metasample_CB,
                              design = ~ mainsite)
dds_CB

## ----prefilter----------------------------------------------------------------
keep <- rowSums(counts(dds_CB) >= 10)
dds_CB <- dds_CB[names(keep),]
dim(dds_CB)
# length(unique(rownames(dds_CB)))
dds_CB$mainsite <- factor(dds_CB$mainsite, 
                          levels = c("Metastasis","Primary"))
...
dim(norm_df)
colnames(counts_df)
sids <- colnames(counts_df)[colnames(counts_df)!='SA919X7XB05604_2']
counts_df <- counts_df[,sids]
norm_df <- norm_df[,sids]
dim(norm_df)
sc <- colSums(counts_df)
sn <-  colSums(norm_df)

stat <- tibble::tibble(library_size=c(sc, sn), sample_type=c(names(sc), names(sn)),
                       processed=c(rep('counts',length(sc)), rep('DESeq2_normalized',length(sn))))
...
p
saveRDS(p, paste0(save_dir, 'DESeq2_normalized.rds'))

save_dir <- paste0(input_dir,'debug/')
# dir.create(save_dir)
counts_df1 <- counts_df %>% as.data.frame()
counts_df1$ens_gene_id <- rownames(counts_df1)
dim(counts_df1)
df_counts_fn <- paste0(save_dir, 'counts_CB.csv.gz')
counts_df <- data.table::fread(df_counts_fn)
head(counts_df)
data.table::fwrite(counts_df1, df_counts_fn)
datatag <- 'SA919_CB'
scran_norm_df <- normalize_by_size_factor(df_counts_fn, datatag, 
                         save_dir, gene_filter_thresh=0,
                         mito_gene_removal=FALSE)
scran_norm_df <- data.table::fread(paste0(save_dir, 'SA919_CB_sizefactor_normalized.csv.gz'))

dim(scran_norm_df)
head(scran_norm_df)
scran_norm_df1 <- scran_norm_df
scran_norm_df1$ens_gene_id <- NULL
sc <- colSums(counts_df)
sn <-  colSums(scran_norm_df1)

stat_scran <- tibble::tibble(library_size=c(sc, sn), sample_type=c(names(sc), names(sn)),
                       processed=c(rep('counts',length(sc)), rep('Scran_normalized',length(sn))))


p1 <- ggplot(stat_scran, aes(sample_type, y=library_size, fill=sample_type)) + #
  facet_grid(~processed) + 
  theme_bw() + 
  theme(axis.text.x = element_text(size=10, angle = 90),
        legend.position = 'none')
...
p1
saveRDS(p, paste0(input_dir, 'Scran_normalized.rds'))


p_total <- cowplot::plot_grid(p, p1)
png(paste0(save_dir,datatag,"_compare_norm_methods.png"), 
    height = 600, width=2*850, res = 2*72)
print(p_total)
dev.off()
...
res_CB <- results(dds_CB, contrast=c("mainsite","Metastasis","Primary"))
dim(res_CB)
head(res_CB)
res_CB$ensembl_gene_id <- rownames(res_CB)
length(unique(rownames(res_CB)))
res_CB$pvalue
res_CB <- res_CB %>%
  as.data.frame() %>%
  dplyr::filter(abs(log2FoldChange)>=0.5 & pvalue<0.05)
dim(res_CB)  



## Get cnv genes, get logFC, plot logFC distribution
cnv <- data.table::fread(paste0(input_dir, 'SA919_DE_analysis/mapping_gene_cnv_SA919.csv.gz'))  
dim(cnv)
cnv <- cnv[!duplicated(cnv$ensembl_gene_id),]
head(cnv)

rownames(cnv) <- cnv$ensembl_gene_id
rv <- rowVars(as.matrix(cnv[,c(5, 6)]))
length(rv)
...
# View(cnv)
sum(cnv$B < cnv$C)
# g_used <- gsub('\\.','',res_CB$ensembl_gene_id)
scran_backup <- scran_norm_df
scran_norm_df$ens_gene_id <- get_geneId_without_version(scran_norm_df$ens_gene_id)
sum(cnv$ensembl_gene_id %in% scran_norm_df$ens_gene_id)
cnv_genes <- intersect(cnv$ensembl_gene_id, scran_norm_df$ens_gene_id)
length(cnv_genes)
scran_norm_df1 <- scran_norm_df
scran_norm_df1 <- scran_norm_df1 %>%
  dplyr::filter(ens_gene_id %in% cnv_genes)
head(scran_norm_df1)
keep <- rowSums(scran_norm_df1[,c(1,2,3)]) >= 10
summary(as.factor(keep))
length(keep)
scran_norm_df1 <- scran_norm_df1[keep,]

dim(scran_norm_df1)

eps <- 0.01
scran_norm_df1$log2FC <- log2(scran_norm_df1$SA919X7XB05604 + eps) 
- log2(mean(scran_norm_df1$SA919X7XB05378 + scran_norm_df1$SA919X7XB05402) + eps) 

summary(scran_norm_df1$log2FC)

scran_norm_df1 <- scran_norm_df1 %>%
  dplyr::mutate(gene_type=
                case_when(
                  log2FC>0 ~ 'up_regulated',
                  TRUE ~ 'down_regulated'))
scran_norm_df1 <- scran_norm_df1 %>%
  dplyr::mutate(log2FC=
                  case_when(
...
scran_norm_df2 <- scran_norm_df1 %>%
  left_join(st, by='gene_type') %>%
  mutate(gene_type=paste0(gene_type, ' (',nb_genes,' genes)'))
sum(scran_norm_df2$log2FC>0)
p_cis <- ggplot(scran_norm_df2, aes(x=gene_type, y=log2FC)) +
         geom_jitter(width=0.5, aes(color=gene_type)) + 
         theme_bw() + 
         theme(axis.text.x = element_text(size=10, angle = 90),
               legend.position = 'none') +
         scale_y_continuous(breaks = rep(round(min(scran_norm_df2$log2FC)):
                                           round(max(scran_norm_df2$log2FC)),1))

p_cis

png(paste0(save_dir,datatag,"_cis_genes.png"), 
    height = 850, width=2*300, res = 2*72)
print(p_cis)
dev.off()
