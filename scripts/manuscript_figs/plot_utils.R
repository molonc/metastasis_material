...
get_obs_genes <- function(obs_clones){
  ...
  obs_genes <- intersect(meta_genes$ens_gene_id, cnv$ensembl_gene_id)
  length(unique(obs_genes))
  # res <- list(obs_genes=obs_genes, total_genes=meta_genes$symbol)
  return(obs_genes)  
}

get_cis_genes <- function(obs_genes, obs_clones){
  script_dir <- "/Users/hoatran/Documents/projects_BCCRC/hakwoo_project/code/metastasis_material/materials/bulkRNAseq/"
  cnv <- data.table::fread(paste0(script_dir, 'SA919_DE_analysis_DESeq2_Hoa_09April2024/mapping_gene_cnv_SA919.csv.gz'))  
  cnv <- cnv[!duplicated(cnv$ensembl_gene_id),]
  dim(cnv)
  ...
  rv <- matrixStats::rowVars(as.matrix(cnv_vals))  # median copy number profile of obs clones
...
plot_gex_cnv_v2 <- function(deg_df,
  ...
  annots <- annotables::grch38 %>%
        dplyr::select(ens_gene_id = ensgene, start, end) #, chr
  annots <- annots[!duplicated(annots),]
  df_track <- deg_df %>%
    inner_join(annots, by=c('ens_gene_id')) %>%
    dplyr::filter(chr %in% chrs) %>%
  ...
  if(!is.null(additional_genes_symb)) {
    df_annot <- df_track  %>% 
      dplyr::filter(symbol %in% additional_genes_symb)
    # df_annot <- df_annot  %>% 
    #   bind_rows(dplyr::filter(df_track, symbol %in% additional_genes_symb))  %>% 
    #   distinct()
  ...
  track_plot2 <- track_plot +
    ...
    geom_text_repel(data = df_annot, aes(label = symbol), size = 5,   # was 3.3
                    min.segment.length = 0, max.overlaps = Inf,
                    color='black', segment.alpha = 0.2,segment.curvature = 1, segment.angle = 20,
                    family = my_font)
...
plot_CNV <- function(df_cnv_fn, clones, meta_genes=NULL){
  ...
  if(is.null(meta_genes)){
    ...
    df_cnv <- df_cnv %>%
      dplyr::filter(ensembl_gene_id %in% meta_genes) #meta_genes$ensembl_gene_id
  }
  ...
  # cnv_colors <- c('#4880B8', '#A7C9DF','#CCCCCC','#F5CE93',
  # '#ED9364','#D2553E','#A42116','#8B1A43',
  # '#CB3576','#D06CAD','#C196C4','#D0BAD8')
